...
public:
  ...
  const Vector3 getHalfExtent() const {
    return (max_ - min_) / 2;
  }

  FloatType getHalfExtent(std::size_t index) const {
    return (max_ - min_)(index) / 2;
  }
...
public:
  ...
  std::array<Triangle, 2> splitTriangleInto2() const {
    const Triangle can_tri = getCanonicalTriangle();
  ...
  std::array<Triangle, 3> splitTriangleInto3() const {
    const Vector3 center = getCenter();
    const Triangle tri1(v1(), v2(), center);
    const Triangle tri2(v2(), v3(), center);
    const Triangle tri3(v3(), v1(), center);
    return std::array<Triangle, 3> { tri1, tri2, tri3 };
  }

  bool intersects(const bh::BoundingBox3D<FloatT>& bbox) const {
    FloatT boxcenter[3];
    FloatT boxhalfsize[3];
    FloatT triverts[3][3];
    boxcenter[0] = bbox.getCenter()(0);
    boxcenter[1] = bbox.getCenter()(1);
    boxcenter[2] = bbox.getCenter()(2);
    boxhalfsize[0] = bbox.getHalfExtent(0);
    boxhalfsize[1] = bbox.getHalfExtent(1);
    boxhalfsize[2] = bbox.getHalfExtent(2);
    triverts[0][0] = v1()(0);
    triverts[0][1] = v1()(1);
    triverts[0][2] = v1()(2);
    triverts[1][0] = v2()(0);
    ...
    triverts[2][1] = v3()(1);
    triverts[2][2] = v3()(2);
    const bool result = triBoxOverlap(boxcenter, boxhalfsize, triverts);
    return result;
  }
...
using PolygonWithLowerAndUpperPlanef = PolygonWithLowerAndUpperPlane<float>;
using PolygonWithLowerAndUpperPlaned = PolygonWithLowerAndUpperPlane<double>;
